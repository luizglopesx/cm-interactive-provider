...
type ButtonStruct struct {
	...
	// Deprecated: use `imageUrl` instead. Kept as a fallback for `imageUrl` for backwards compatibility.
...
func (s *sendService) SendButton(data *ButtonStruct, instance *instance_model.Instance) (*MessageSendStruct, error) {
	...
	// thumbnailUrl is a legacy alias for imageUrl, kept for backwards compatibility.
	headerImageUrl := data.ImageUrl
	if headerImageUrl == "" {
		headerImageUrl = data.ThumbnailUrl
	}
	...
	if hasReply && !hasOtherTypes && !hasPix {
		...
		if headerImageUrl != "" {
			if resp, err := http.Get(headerImageUrl); err == nil {
		...
		// Header com imagem quando imageUrl/thumbnailUrl é fornecido (paridade com Evolution 2.3.7)
		if headerImageUrl != "" {
			if thumbResp, thumbErr := http.Get(headerImageUrl); thumbErr == nil {
				thumbData, readErr := io.ReadAll(thumbResp.Body)
				thumbResp.Body.Close()
				if readErr == nil {
					if uploaded, uploadErr := client.Upload(context.Background(), thumbData, whatsmeow.MediaImage); uploadErr == nil {
						jpegThumb := makeJPEGThumbnail(thumbData, 72)
...
func (s *sendService) SendList(data *ListStruct, instance *instance_model.Instance) (*MessageSendStruct, error) {
	...
	// type MUST be "product_list" regardless of the protobuf ListType — WhatsApp
	// server rejects "single_select" with error 405 (see docs/local-interactive-go-test-runbook.md).
...
func (s *sendService) SendMessage(instance *instance_model.Instance, msg *waE2E.Message, messageType string, data *SendDataStruct) (*MessageSendStruct, error) {
	...
	// Injetar nodes biz/bot customizados (PIX, botões interativos, carrossel, listas, etc.)
	// no stanza XMPP. Cada Send* (SendButton/SendList/SendCarousel) monta seu próprio node,
	// necessário para que o WhatsApp entregue o NativeFlowMessage.
...
func (s *sendService) SendCarousel(data *CarouselStruct, instance *instance_model.Instance) (*MessageSendStruct, error) {
	...
	// Injetar nó <biz> — necessário para que o WhatsApp entregue o NativeFlowMessage do
	// carrossel (equivalente ao buildInteractiveBizNode do Evolution API 2.3.7/Baileys).
	// SendMessage não injeta mais isso automaticamente; cada Send* passa o seu.
	carouselBizNodes := []waBinary.Node{
		{
			Tag: "biz",
			Content: []waBinary.Node{
				{
	...
	message, err := s.SendMessage(instance, msg, "InteractiveMessage", &SendDataStruct{
		Number:          data.Number,
		Delay:           data.Delay,
		AdditionalNodes: &carouselBizNodes,
	})
